...
def cluster_embeddings_with_progress(embeddings, n_clusters=10, method="kmeans", eps=0.5, min_samples=5, linkage="ward"):
    """
...
async def main():
    """
    Main function to orchestrate the comment analysis process.
    """
    logging.info("Starting the main process.")

    # Step 0: Fetch comments
    logging.debug("Fetching YouTube comments...")
    comments = await get_youtube_comments(VIDEO_ID, max_comments=100)
    logging.info(f"Fetched {len(comments)} comments.")

    # Step 1: Generate embeddings
    logging.debug("Generating embeddings for comments...")
    embeddings = await get_embeddings(comments)
    logging.info(f"Generated embeddings for {len(embeddings)} comments.")

    # Step 2: Cluster embeddings
    logging.debug("Clustering embeddings...")
    labels = cluster_embeddings_with_progress(
        embeddings,
        n_clusters=10,
        method="hierarchical",
        linkage="ward"
    ...
    logging.debug("Extracting keywords from clusters...")
    ...
    generate_wordcloud(cluster_keywords)
